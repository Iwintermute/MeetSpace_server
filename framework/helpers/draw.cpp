...
ImVec2 text_size_animed(const char* text, float size, const char* text_end, bool hide_text_after_double_hash, float wrap_width)
{
...
void c_draw::text_animed(ImDrawList* draw_list, ImFont* font, float size, const ImVec2& pos_min, const ImVec2& pos_max, ImU32 color, const char* text, const char* text_display_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    PushFont(font);

    ImVec2 pos = pos_min;
    const ImVec2 text_size = text_size_if_known ? *text_size_if_known : text_size_animed(text, size, text_display_end, false, 0.0f);
    ...
    {
        ImVec4 fine_clip_rect(clip_min->x, clip_min->y, clip_max->x, clip_max->y);
        draw_list->AddText(font, size, pos, color, text, text_display_end, 0.0f, &fine_clip_rect);
    }
    else
    {
        draw_list->AddText(font, size, pos, color, text, text_display_end, 0.0f, NULL);
    }
    PopFont();
}
