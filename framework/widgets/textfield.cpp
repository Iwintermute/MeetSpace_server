...
bool text_field_ex(std::string_view label, std::string_view hint, char* buf, int buf_size, float text_padding, float line_padding, ImU32 text_col, bool centered, const ImRect& rect, bool* active, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data, bool fg = false)
{
    ...
    {
        const ImFontGlyph* glyphsadiufisadf = g.Font->FindGlyph('H');
        ImFontGlyph* glyph = const_cast<ImFontGlyph*>(glyphsadiufisadf);
        glyph->AdvanceX = SCALE(10);
    ...
    gui->easing(animstate->alpha, strlen(buf_display) > 0 ? 0.f : *active ? 0.f : 1.f, 6.f, static_easing);
    ...
    *active = render_cursor;
    ...
    if (!is_multiline)
    {
        draw->text_clipped(window->DrawList, font->get(inter_medium_data, 13), rect.Min + ImVec2(text_padding + SCALE(elements->textfield.size.x / 2 * (1 - animstate->alpha)), 0), rect.Max, draw->get_clr(clr->main.text_inactive, animstate->alpha), hint.data(), NULL, NULL, ImVec2(0.f, 0.5f));
    }
...
bool c_widgets::text_field(std::string_view name, std::string_view icon, char* buf, int size, bool* enter_pressed, ImVec2 textfieldsize)
{
    struct field_state
    {
        bool active{ false };
        bool value_changed{ false };
        ImVec4 icon{ clr->main.text_inactive };
    };
    ...
    field_state* state = gui->anim_container<field_state>(id + window->GetID("textfield"));
    ...
    gui->easing(state->icon, hasContent ? clr->main.accent.Value : clr->main.text_inactive.Value, 12.f, dynamic_easing);

    // Modern input field styling
    bool is_focused = state->active;
    ImU32 bg_color = draw->get_clr(is_focused ? clr->textfield.background_focus : clr->textfield.background);
    ImU32 border_color = draw->get_clr(is_focused ? clr->textfield.border_focus : clr->textfield.border);
    
    // Background
    draw->rect_filled(window->DrawList, total.Min, total.Max, bg_color, SCALE(elements->textfield.rounding));
    
    // Border
    draw->rect(window->DrawList, total.Min, total.Max, border_color, SCALE(elements->textfield.rounding), 
        draw_flags_round_corners_all, SCALE(1));
    
    // Icon background
    draw->rect_filled(window->DrawList, rect.Min, rect.Max, 
        draw->get_clr(clr->textfield.icon_background), SCALE(elements->textfield.rounding));
    draw->text_clipped(window->DrawList, font->get(icons_data, 15), rect.Min, rect.Max, draw->get_clr(state->icon), icon.data(), NULL, NULL, ImVec2(0.5f, 0.5f));
    ...
    // If user wants to detect enter, we must enable the flag
    if (enter_pressed)
        flags |= ImGuiInputTextFlags_EnterReturnsTrue;

    gui->push_font(icon.data() == "C" ? font->get(icons_data, 4.f) : font->get(inter_medium_data, 13.f));
    bool res = text_field_ex(name.data(), name, buf, size, SCALE(elements->window.padding.x), SCALE(elements->window.padding.x), draw->get_clr(clr->main.text), false, text_zone, &state->active, flags, 0, 0);
    gui->pop_font();
