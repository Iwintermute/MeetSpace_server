...
{
    if (ImRect{ gui->window_pos(), gui->window_pos() + ImVec2(gui->window_width(), SCALE(elements->top_bar.product_height)) }.Contains(gui->mouse_pos()) && gui->mouse_clicked(mouse_button_left))
        var->gui.dragging = true;

    if (!gui->mouse_down(mouse_button_left))
        var->gui.dragging = false;

    if (var->gui.dragging || var->gui.update_window)
    {
        GetWindowRect(hwnd, &rc);
        MoveWindow(hwnd, rc.left + GetMouseDragDelta().x, rc.top + GetMouseDragDelta().y, GetWindowSize().x, GetWindowSize().y, FALSE);
        var->gui.update_window = false;
    }
}
...
void c_gui::gradient(std::string_view name, bool active, ImVec4 col[3])
{
    struct anim_state
    {
        ImVec4 target_rect[3];
    };

    anim_state* state = gui->anim_container<anim_state>(gui->get_window()->GetID(name.data()));
 
    if (active)
    {
        float time = ImGui::GetTime();
        float base_hue = fmod(time * 0.3f, 1.0f);

        state->target_rect[0] = ImColor::HSV(base_hue, 0.15f, 0.98f);
        state->target_rect[1] = ImColor::HSV(fmod(base_hue + 0.12f, 1.0f), 0.15f, 0.98f);
        state->target_rect[2] = ImColor::HSV(fmod(base_hue + 0.24f, 1.0f), 0.15f, 0.98f);
    }
    else
    {
        state->target_rect[0] = clr->main.accent.Value;
        state->target_rect[1] = clr->main.grad_1.Value;
        state->target_rect[2] = clr->main.grad_2.Value;
    }

    gui->easing(col[0], state->target_rect[0], 12.f, dynamic_easing);
    gui->easing(col[1], state->target_rect[1], 12.f, dynamic_easing);
    gui->easing(col[2], state->target_rect[2], 12.f, dynamic_easing);
}

std::vector<std::string> wrap_text(ImFont* font, float max_width, const std::string& text)
{
    ...
    {
        const char* line_end = font->CalcWordWrapPositionA(1.0f, line_start, text_end, max_width);

        if (line_end == line_start)
            line_end = text_end;

        lines.push_back(std::string(line_start, line_end));
        line_start = line_end;
...
void c_gui::log_notify(int type)
{
    gui->begin_content("log_notify", SCALE(elements->log_notify.size));
    {
        ...
        draw->text_clipped(drawlist, font->get(icons_data, 40.f), top_bar.Min + SCALE(0, elements->log_notify.top_padding - 13), top_bar.Max, draw->get_clr(clr->main.accent), "A", NULL, NULL, ImVec2(0.5f, 0.f));
        draw->text_clipped(drawlist, font->get(inter_semibold_data, 13), top_bar.Min, top_bar.Max - SCALE(0, elements->log_notify.top_padding - 2), draw->get_clr(clr->main.text), type == 1 ? "The login or password is incorrect." : "Interaction error, this account does not exist", NULL, NULL, ImVec2(0.5f, 1.f));
        ...
        {
            float text_size{ 13 };
            float pulse_size{ 18.f };
            bool pulse_direction{ true };
        };

        anim_state* state = gui->anim_container<anim_state>(gui->get_window()->GetID("log_notify_pulse"));

        float target_size = state->pulse_direction ? 30.f : 15.f;
        gui->easing(state->pulse_size, target_size, 16.f, dynamic_easing);
        ...
        {
            draw->text_clipped(drawlist, font->get(inter_medium_data, 13), desc_pos, content.Max, draw->get_clr(clr->main.text_inactive), line.c_str(), NULL, NULL, ImVec2(0.5f, 0.f));
            desc_pos.y += gui->text_size(font->get(inter_medium_data, 13), "A").y;
        }

        if (!total.Contains(gui->mouse_pos()) && (gui->mouse_clicked(mouse_button_left) || gui->mouse_clicked(mouse_button_right)))
            elements->log_notify.type = 0;
    }
    gui->end_content();
}
void c_gui::loading()
{
    struct loading_state
    {
        ...
        ImVec4 line[3]{ clr->main.accent, clr->main.grad_1, clr->main.grad_2 };
    ...
    loading_state* state = gui->anim_container<loading_state>(gui->get_window()->GetID("loading"));

    gui->begin_content("loading_content", SCALE(elements->loading.size));
    {
        ImDrawList* drawlist = gui->window_drawlist();

        const ImRect total(gui->window_pos(), gui->window_pos() + gui->window_size());
        const ImRect top_bar(total.Min, ImVec2(total.Max.x, total.Min.y + SCALE(elements->log_notify.top_height)));
        const ImRect button(ImVec2(total.Min.x + SCALE(elements->window.padding.x), top_bar.Max.y) + SCALE(state->rect_offset, state->rect_offset / 2), ImVec2(total.Max.x - SCALE(elements->window.padding.x), top_bar.Max.y + SCALE(elements->loading.button_height)) - SCALE(state->rect_offset, state->rect_offset / 2));
        const ImRect line(ImVec2(button.Min.x, total.Max.y - SCALE(elements->window.padding.y + elements->loading.line_height)), ImVec2(button.Max.x, total.Max.y - SCALE(elements->window.padding.y)));

        draw->rect_filled(drawlist, total.Min, total.Max, draw->get_clr(clr->window.background), SCALE(elements->log_notify.window_rounding));

        draw->text_clipped(drawlist, font->get(icons_data, 40.f), top_bar.Min + SCALE(0, elements->log_notify.top_padding - 13), top_bar.Max, draw->get_clr(clr->main.accent), "A", NULL, NULL, ImVec2(0.5f, 0.f));
        draw->text_clipped(drawlist, font->get(inter_semibold_data, 13), top_bar.Min, top_bar.Max - SCALE(0, elements->log_notify.top_padding - 2), draw->get_clr(clr->main.text), "Are you sure you want to start the injection?", NULL, NULL, ImVec2(0.5f, 1.f));
        ...
        state->hovered = button.Contains(gui->mouse_pos());

        if ((state->hovered && gui->mouse_released(mouse_button_left)))
            state->clicked = true;

        gui->easing(state->alpha, state->hovered || state->clicked || held ? 1.f : 0.f, 5.f, static_easing);
        gui->easing(state->text, state->hovered || state->clicked || held ? clr->main.black.Value : clr->main.text.Value, 12.f, dynamic_easing);
        gui->easing(state->text_size, state->hovered && ! state->clicked ? 15.f : 13.f, 16.f, dynamic_easing);
        gui->easing(state->text_offset, state->hovered && !state->clicked ? 2.f : 1.f, 16.f, static_easing);
        gui->easing(state->rect_offset, state->clicked || held ? 4.f : 0.f, 14.f, dynamic_easing);
        gui->easing(elements->loading.size.y, state->clicked ? elements->window.padding.y * 2 + elements->loading.line_height + elements->loading.button_height + elements->log_notify.top_height : elements->window.padding.y + elements->loading.button_height + elements->log_notify.top_height, 14.f, dynamic_easing);
        gui->gradient("four", held, state->rect);
        if(state->clicked)
        {
            if(state->timer < 1.f)
                state->timer += gui->fixed_speed(0.1f);

            gui->gradient("five", true, state->line);

            draw->rect_filled(drawlist, line.Min, line.Max, draw->get_clr(clr->messages.line_background), line.GetHeight());
            draw->rect_filled_multi_color(drawlist, line.Min, ImVec2(line.Min.x + line.GetWidth() * state->timer / 2 + SCALE(1), line.Max.y),
                draw->get_clr(state->line[0]),
                draw->get_clr(state->line[1]),
                draw->get_clr(state->line[1]),
                draw->get_clr(state->line[0]),
                line.GetHeight(), draw_flags_round_corners_left);

            draw->rect_filled_multi_color(drawlist, ImVec2(line.Min.x + line.GetWidth() * state->timer / 2 - SCALE(1), line.Min.y), ImVec2(line.Min.x + line.GetWidth() * state->timer, line.Max.y),
                draw->get_clr(state->line[1]),
                draw->get_clr(state->line[2]),
                draw->get_clr(state->line[2]),
                draw->get_clr(state->line[1]),
                line.GetHeight(), draw_flags_round_corners_right);
        }
        if (state->timer >= 0.999f)
            var->gui.loading = false;

        if(!var->gui.loading && elements->loading.offset >= var->window.size.y - SCALE(5))
        {
            state->alpha = 0;
            state->clicked = false;
            state->hovered = false;
        ...
        draw->rect(drawlist, button.Min, button.Max, draw->get_clr(clr->button.card_outline), SCALE(elements->button.rounding));

        draw->rect_filled_multi_color(drawlist, button.Min, ImVec2(button.GetCenter().x, button.Max.y),
            draw->get_clr(state->rect[0], state->alpha),
            draw->get_clr(state->rect[1], state->alpha),
            draw->get_clr(state->rect[1], state->alpha),
            draw->get_clr(state->rect[0], state->alpha),
            SCALE(elements->button.rounding), draw_flags_round_corners_left);

        draw->rect_filled_multi_color(drawlist, ImVec2(button.GetCenter().x, button.Min.y), button.Max,
            draw->get_clr(state->rect[1], state->alpha),
            draw->get_clr(state->rect[2], state->alpha),
            draw->get_clr(state->rect[2], state->alpha),
            draw->get_clr(state->rect[1], state->alpha),
            SCALE(elements->button.rounding), draw_flags_round_corners_right);

        draw->text_animed(drawlist, font->get(inter_semibold_data, 13), SCALE(state->text_size), button.Min, button.Max, draw->get_clr(state->text), "Start session", NULL, NULL, ImVec2(0.5f, 0.5f));
...
void c_gui::draw_decorations()
{
    ...
    gui->easing(elements->window.main_content_padding, elements->top_bar.log_line_alpha <= 0.01 ? elements->top_bar.product_height : elements->log_page.top_height, 14.f, dynamic_easing);

    draw->image_rounded(drawlist, var->gui.logo_back, pos, pos + size, ImVec2(0, 0), ImVec2(1, 1), draw->get_clr({ 1.f, 1.f, 1.f, 1.f }), var->style.window_rounding);
    draw->rect_filled(drawlist, pos + SCALE(0, elements->window.main_content_padding), pos + size, draw->get_clr(clr->window.background), var->style.window_rounding, draw_flags_round_corners_bottom);

    draw->rect_filled(drawlist, pos + ImVec2((size.x - SCALE(elements->top_bar.rect_size.x)) / 2, SCALE(elements->log_page.top_height - elements->top_bar.rect_size.y / 2 + (elements->top_bar.pos_y - (elements->log_page.top_height - elements->top_bar.size.y / 2)))), pos + ImVec2((size.x + SCALE(elements->top_bar.rect_size.x)) / 2, SCALE(elements->log_page.top_height + elements->top_bar.rect_size.y / 2 + (elements->top_bar.pos_y - (elements->log_page.top_height - elements->top_bar.size.y / 2)))), draw->get_clr(clr->window.background), SCALE(elements->top_bar.def_rounding));

    {
        const ImRect top_bar(pos + ImVec2((size.x - SCALE(elements->top_bar.size.x)) / 2, SCALE(elements->top_bar.pos_y)), pos + ImVec2((size.x + SCALE(elements->top_bar.size.x)) / 2, SCALE(elements->top_bar.pos_y + elements->top_bar.size.y)));

        gui->easing(elements->top_bar.pos_y, elements->top_bar.log_line_alpha <= 0.01 ? 0 : elements->log_page.top_height - elements->top_bar.size.y / 2, 16.f, dynamic_easing);
        gui->easing(elements->top_bar.size.x, elements->top_bar.pos_y <= 1 ? var->window.size.x : elements->top_bar.def_size.x, 12.f, dynamic_easing);
        gui->easing(elements->top_bar.size.y, elements->top_bar.log_line_alpha <= 0.01 ? elements->top_bar.product_height : elements->top_bar.def_size.y, 12.f, dynamic_easing);
        gui->easing(elements->top_bar.round_top, elements->top_bar.pos_y <= 1 ? var->window.rounding : elements->top_bar.def_rounding, 12.f, dynamic_easing);
        gui->easing(elements->top_bar.round_bottom, elements->top_bar.pos_y <= 1 ? 0.f : elements->top_bar.def_rounding, 12.f, dynamic_easing);
        gui->easing(clr->top_bar.col.Value, elements->top_bar.product_outline_alpha >= 0.99 ? clr->top_bar.col_end.Value : clr->top_bar.col_start.Value, 12.f, dynamic_easing);

        gui->easing(elements->top_bar.log_line_width, elements->log_page.offset >= var->window.size.x - 1 ? 0.f : SCALE(var->window.size.x / 2), 10.f, dynamic_easing);
        gui->easing(elements->top_bar.log_line_alpha, (elements->top_bar.log_line_width <= elements->top_bar.def_size.y) ? 0.f : 1.f, 6.f, static_easing);

        const int vtx_start_1 = drawlist->VtxBuffer.Size;
        draw->line(drawlist, pos + ImVec2(size.x / 2 - SCALE(elements->top_bar.log_line_width), SCALE(elements->window.main_content_padding)), pos + ImVec2(size.x / 2, SCALE(elements->window.main_content_padding)), draw->get_clr(clr->main.text, elements->top_bar.log_line_alpha), SCALE(1));
        const int vtx_end_1 = drawlist->VtxBuffer.Size;
        draw->set_linear_color_alpha(drawlist, vtx_start_1, vtx_end_1, pos + SCALE(0, elements->window.main_content_padding - 1), pos + ImVec2(size.x / 2, SCALE(elements->window.main_content_padding + 1)), draw->get_clr(clr->main.accent, 0), draw->get_clr(clr->main.grad_1));
        const int vtx_start_2 = drawlist->VtxBuffer.Size;
        draw->line(drawlist, pos + ImVec2(size.x / 2, SCALE(elements->window.main_content_padding)), pos + ImVec2(size.x / 2 + SCALE(elements->top_bar.log_line_width), SCALE(elements->window.main_content_padding)), draw->get_clr(clr->main.text, elements->top_bar.log_line_alpha), SCALE(1));
        const int vtx_end_2 = drawlist->VtxBuffer.Size;
        draw->set_linear_color_alpha(drawlist, vtx_start_2, vtx_end_2, pos + ImVec2(size.x / 2, SCALE(elements->window.main_content_padding - 1)), pos + ImVec2(size.x, SCALE(elements->window.main_content_padding + 1)), draw->get_clr(clr->main.grad_1), draw->get_clr(clr->main.grad_2, 0));

        draw->rect_filled(drawlist, top_bar.Min, ImVec2(top_bar.Max.x, top_bar.GetCenter().y), draw->get_clr(clr->top_bar.col), SCALE(elements->top_bar.round_top), draw_flags_round_corners_top);
        draw->rect_filled(drawlist, ImVec2(top_bar.Min.x, top_bar.GetCenter().y), top_bar.Max, draw->get_clr(clr->top_bar.col), SCALE(elements->top_bar.round_bottom), draw_flags_round_corners_bottom);

        gui->easing(elements->top_bar.product_outline_alpha, elements->top_bar.size.x >= var->window.size.x - 1 ? 1.f : 0, 6.f, static_easing);

        draw->line(drawlist, ImVec2(top_bar.Min.x, top_bar.Max.y - SCALE(1)), ImVec2(top_bar.Max.x, top_bar.Max.y - SCALE(1)), draw->get_clr(clr->button.log_outline, elements->top_bar.product_outline_alpha), SCALE(1));

        {
            gui->easing(elements->top_bar.product_line_width, elements->top_bar.product_outline_alpha >= 0.99 ? elements->top_bar.product_line_def_width : 0.f, 10.f, dynamic_easing);

            draw->rect_filled_multi_color(drawlist, ImVec2(top_bar.GetCenter().x - gui->text_size(font->get(icons_data, 60.f), "A").x / 2 - SCALE(elements->top_bar.product_line_padding), top_bar.GetCenter().y - SCALE(0.5)),
                ImVec2(top_bar.GetCenter().x - gui->text_size(font->get(icons_data, 60.f), "A").x / 2 - SCALE(elements->top_bar.product_line_padding + elements->top_bar.product_line_width), top_bar.GetCenter().y + SCALE(0.5)),
                draw->get_clr(clr->top_bar.product_line_col_1, 0.5), draw->get_clr(clr->window.background, 0), draw->get_clr(clr->window.background, 0), draw->get_clr(clr->top_bar.product_line_col_1, 0.5), 0, draw_flags_round_corners_all);
        ...
        gui->easing(elements->top_bar.font_size, elements->top_bar.product_line_width >= elements->top_bar.product_line_def_width - 1 ? 60.f : 40.f, 12.f, dynamic_easing, true);

        draw->text_animed(drawlist, font->get(icons_data, 40), SCALE(elements->top_bar.font_size), top_bar.Min, top_bar.Max, draw->get_clr(clr->main.accent), "A", NULL, NULL, ImVec2(0.5f, 0.5f));
    }

    gui->easing(elements->button.close_pos, elements->top_bar.size.x >= var->window.size.x - 1 ? SCALE(elements->top_bar.product_height - elements->button.close_size.y) / 2 : SCALE(elements->button.close_padding), 12.f, dynamic_easing);

    gui->set_pos(ImVec2(size.x - SCALE(elements->button.close_size.x) - elements->button.close_pos, elements->button.close_pos), pos_all);
    widgets->close_button();

    gui->easing(elements->button.return_pos, elements->game_card.section_count != -1 ? elements->button.return_def_pos : -elements->button.return_def_pos, 12.f, dynamic_easing);

    gui->set_pos(SCALE(elements->button.return_pos, (elements->top_bar.size.y - elements->button.return_size.y) / 2), pos_all);
...
{
    font->get(inter_semibold_data, 13);
    cfg->init_config();
    notify->setup_notify();
}
