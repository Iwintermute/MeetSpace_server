...
void c_messenger_widgets::render_login_screen()
{
    ...
    widgets->text_field("Phone", "A", app_state->login_phone, sizeof(app_state->login_phone));
    ...
    widgets->text_field("Password", "B", app_state->login_password, sizeof(app_state->login_password));
    ...
    ImVec4 target_bg = sign_hovered ? clr->button.primary_hover.Value : clr->button.primary.Value;
    gui->easing(state->button_bg, target_bg, 12.f, dynamic_easing);
    gui->easing(state->button_alpha, 1.0f, 8.f, static_easing);

    ImU32 sign_bg = draw->get_clr(state->button_bg, state->button_alpha);
    ...
    draw->rect_filled(window->DrawList, sign_rect.Min, sign_rect.Max,
        sign_bg, SCALE(12));
...
void c_messenger_widgets::render_registration_screen()
{
    ...
    draw->text_clipped(window->DrawList, font->get(icons_data, 22),
        avatar_center - ImVec2(avatar_radius, avatar_radius),
        avatar_center + ImVec2(avatar_radius, avatar_radius),
        draw->get_clr(clr->button.text), "+", nullptr, nullptr, ImVec2(0.5f, 0.5f));
    ...
    widgets->text_field("Phone Number", "A", app_state->phone_input, sizeof(app_state->phone_input));
    ...
    ImVec4 target_bg = send_hovered ? clr->button.primary_hover.Value : clr->button.primary.Value;
    gui->easing(state->button_bg, target_bg, 12.f, dynamic_easing);
    gui->easing(state->button_alpha, 1.0f, 8.f, static_easing);

    ImU32 send_bg = draw->get_clr(state->button_bg, state->button_alpha);
    ...
    draw->rect_filled(window->DrawList, send_rect.Min, send_rect.Max,
        send_bg, SCALE(12));
...
void c_messenger_widgets::render_phone_verification_screen()
{
    ...
    ImVec4 target_bg = hovered ? clr->button.primary_hover.Value : clr->button.primary.Value;
    gui->easing(state->button_bg, target_bg, 12.f, dynamic_easing);
    gui->easing(state->button_alpha, 1.0f, 8.f, static_easing);

    ImU32 verify_bg_color = draw->get_clr(state->button_bg, state->button_alpha);
    ...
    if (hovered)
    {
        draw->shadow_rect(window->DrawList, button_rect.Min, button_rect.Max,
            draw->get_clr(ImVec4(0, 0, 0, 0.15f)), SCALE(8), ImVec2(0, SCALE(2)),
            draw_flags_round_corners_all, button_rounding);
    }
    
    draw->rect_filled(window->DrawList, button_rect.Min, button_rect.Max,
        verify_bg_color, button_rounding);
...
void c_messenger_widgets::render_main_messenger()
{
    ...
    // Рендерим правую панель только если анимированная ширина больше 1 пикселя
    if (app_state->right_panel_anim_width > 1.0f && app_state->selected_chat_id >= 0)
    {
        render_right_panel();
    }
...
void c_messenger_widgets::render_left_panel()
{
    ...
    // Modern conference button with smooth transitions
    ImU32 conf_bg_color = draw->get_clr(conf_hovered ? clr->conference.primary : clr->button.secondary);
    ...
    draw->circle_filled(window->DrawList, conf_btn_rect.GetCenter(), SCALE(17), conf_bg_color, 32);
    ...
    ImU32 create_bg_color = draw->get_clr(create_hovered ? clr->button.primary : clr->button.secondary);
    ...
    draw->circle_filled(window->DrawList, create_btn_rect.GetCenter(), SCALE(17), create_bg_color, 32);
    ...
    draw->text_clipped(window->DrawList, font->get(icons_data, 16),
        ...
        icon_color, "+", nullptr, nullptr, ImVec2(0.5f, 0.5f));
...
void c_messenger_widgets::render_center_panel()
{
    ...
    // Используем анимированную ширину для плавного перехода
    float right_width = SCALE(app_state->right_panel_anim_width * t);
    ...
    // Анимация ширины правой панели
    float target_width = app_state->show_right_panel ? app_state->right_panel_width : 0.0f;
    gui->easing(app_state->right_panel_anim_width, target_width, 12.f, dynamic_easing);
    ...
    draw->text_clipped(window->DrawList, font->get(inter_medium_data, 18),
    ...
    // Поле ввода
    gui->set_pos(input_pos + SCALE(20, 20), pos_all);
    widgets->text_field("Type a message", "M", app_state->message_input, sizeof(app_state->message_input));

    // Кнопка отправки
    ImVec2 send_btn_pos = ImVec2(input_pos.x + input_size.x - SCALE(100), input_pos.y + SCALE(20));
    ImRect send_btn_rect(send_btn_pos, send_btn_pos + SCALE(80, 40));
    ...
    // Modern send button
    ImU32 send_bg_color = draw->get_clr(send_hovered ? clr->button.primary_hover : clr->button.primary);
    ...
    draw->rect_filled(window->DrawList, send_btn_rect.Min, send_btn_rect.Max,
        send_bg_color, SCALE(8));
...
// ============================================================================
void c_messenger_widgets::render_right_panel()
{
    ImGuiWindow* window = gui->get_window();
    if (window->SkipItems) return;
    ...
    // Используем анимированную ширину
    float panel_width = SCALE(app_state->right_panel_anim_width);
    
    ImVec2 panel_pos = ImVec2(screen_size.x - panel_width, 0);
    ImVec2 panel_size = ImVec2(panel_width, screen_size.y);

    // Clipping для предотвращения отрисовки за пределами панели
    window->DrawList->PushClipRect(panel_pos, panel_pos + panel_size, true);

    // Modern right panel background
    draw->rect_filled(window->DrawList, panel_pos, panel_pos + panel_size,
        draw->get_clr(clr->messages.background), 0);

    // Modern divider - subtle left border
    draw->rect_filled(window->DrawList,
        ImVec2(panel_pos.x, panel_pos.y),
        ImVec2(panel_pos.x + SCALE(1), panel_pos.y + panel_size.y),
        draw->get_clr(clr->main.divider), 0);

    Chat* chat = app_state->GetChat(app_state->selected_chat_id);
    if (!chat)
    {
        // Если по какой‑то причине чата нет, нужно снять ClipRect перед выходом,
        // иначе стек ImGui испортится и приложение может упасть.
    ...
    // Заголовок
    draw->text_clipped(window->DrawList, font->get(inter_bold_data, 16),
        panel_pos + SCALE(20, 20), panel_pos + ImVec2(panel_width - SCALE(20), SCALE(50)),
        draw->get_clr(clr->main.text), "Chat Info", nullptr, nullptr, ImVec2(0.0f, 0.5f));

    // Информация о чате
    gui->set_pos(panel_pos + SCALE(20, 70), pos_all);
    gui->begin_content("chat_info", ImVec2(panel_width - SCALE(40), screen_size.y - SCALE(90)), SCALE(0, 0), SCALE(0, 15));
    {
        // ВАЖНО: Получаем текущее окно (child), так как begin_content создает новое окно
        ImGuiWindow* child_window = gui->get_window();
        
        // Название чата
        ImVec2 content_pos = child_window->DC.CursorPos;
        draw->text_clipped(child_window->DrawList, font->get(inter_medium_data, 12),
            content_pos, content_pos + ImVec2(panel_width - SCALE(60), SCALE(20)),
            draw->get_clr(clr->main.text_secondary), "Name:", nullptr, nullptr, ImVec2(0.0f, 0.5f));

        gui->dummy(SCALE(0, 25));
        content_pos = child_window->DC.CursorPos;
        
        std::string chat_name = chat_manager->GetChatDisplayName(*chat);
        draw->text_clipped(child_window->DrawList, font->get(inter_medium_data, 14),
            content_pos, content_pos + ImVec2(panel_width - SCALE(60), SCALE(25)),
            draw->get_clr(clr->main.text), chat_name.c_str(), nullptr, nullptr, ImVec2(0.0f, 0.5f));

        // Участники
        gui->dummy(SCALE(0, 45));
        content_pos = child_window->DC.CursorPos;
        draw->text_clipped(child_window->DrawList, font->get(inter_medium_data, 12),
            content_pos, content_pos + ImVec2(panel_width - SCALE(60), SCALE(20)),
            draw->get_clr(clr->main.text_secondary), "Members:", nullptr, nullptr, ImVec2(0.0f, 0.5f));

        gui->dummy(SCALE(0, 30));
        for (int user_id : chat->participant_ids)
        {
            User* user = app_state->GetUser(user_id);
            if (!user && user_id == 0)
            ...
            {
                content_pos = child_window->DC.CursorPos;
                draw->text_clipped(child_window->DrawList, font->get(inter_medium_data, 13),
                    content_pos, content_pos + ImVec2(panel_width - SCALE(60), SCALE(25)),
    ...
    // Восстанавливаем clipping (для родительского окна, так как DrawList родителя использовался ранее)
    // Но так как мы использовали child_window->DrawList внутри, нам не нужно это делать?
    // text_clipped использует PushClipRect / PopClipRect на DrawList.
    // begin_content использует BeginChild.
    // Если render_right_panel использовал window->DrawList ранее (до begin_content), он мог сделать PushClipRect.
    // window->DrawList->PopClipRect() в конце функции относится к "Chat Info" заголовку?
    // Нет, render_right_panel не делал PushClipRect явно, только begin_content делает.
    // Но строка 645 была window->DrawList->PopClipRect(). Это странно.
    // Возможно text_clipped "Chat Info" (строка 596) или что-то еще требовало этого?
    // Нет, text_clipped сам делает Pop.
    // Оставим как есть строки за пределами блока.
    window->DrawList->PopClipRect();
}
